<?php

return [
    'camera_count' => 'Количество камер: :count',
    'cameras' => 'Камеры: :camera_list',
    'period' => 'Отчет сформирован за период с :start по :end',
    'report_generation_datetime' => 'Дата и время формирования отчета: :datetime',
    'operator_name' => 'Имя оператора: :name',
    'report_total_time' => 'Требуемое кол-во часов записи по камерам за все время: :value',
    'report_not_working_camera_count' => 'Кол-во камер не в эксплуатации: :value',
    'report_title_problem_time' => 'Время нахождения камер в режиме Неисправность',
    'report_title_archive_time' => 'Время архива, записанное по камере',
    'report_title_available_time' => 'Время нахождения камер в режиме Штатно',
    'report_title_available_time_percent' => 'Время нахождения камер в режиме Штатно в процентах от требуемого времени',
    'total_value_time' => 'Общее время: :value',
    'total_value_percent' => '% от общего времени: :value',
    'title_datetime' => 'Дата',
    'title_value_time' => 'Часы',
    'title_value_percent' => '%',
    'value_time' => ':hours ч :minutes мин',
    'datetime_format' => 'd.m.Y H:i',
    'date_format' => 'd.m.Y'
];
